...
X_imp_SoftImpute_scaled = StandardScaler().fit_transform(pickle.load(open(my_directory+'X_imp_SoftImpute.pkl','rb'))[None])
...
while True:
    ...
    my_net.fit(X_imp_SoftImpute_scaled , y_IDP_comp)
...
pd.DataFrame(nIDP_names_analysis[my_net.coef_ != 0]).to_csv(my_directory + 'selected_var_names_SoftImpute_LASSO_'+str(n_vars)+'.csv')

pd.DataFrame(nIDP_numbers_analysis[my_net.coef_ != 0]).to_csv(my_directory + 'selected_var_numbers_SoftImpute_LASSO_'+str(n_vars)+'.csv')

my_vars = np.unique([my_str.split('-')[0] for my_str in nIDP_numbers_analysis[my_net.coef_ != 0]])

pd.DataFrame(my_vars).to_csv(my_directory + 'selected_var_numbers_unique_SoftImpute_LASSO_'+str(n_vars)+'.csv')
